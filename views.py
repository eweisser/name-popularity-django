...
def map_from_one_name_NYYS(request, name_input, year_start_input, year_end_input, sex_input, animate_choice):
    ...
    context['maps'] = {}
    context['animate choice'] = animate_choice
    if animate_choice == "separate":
        for yearToMap in range(year_start_input,year_end_input+1):
            ...
            svgInsert = svgCodeGenerator(year_record_list,yearToMap,yearToMap)
        ...
        year_record_list = record_list.filter(year__gte=year_start_input).filter(year__lte=year_end_input)

        if sex_input:
            year_record_list = year_record_list.filter(sex=sex_input)
        else:
            year_record_list = check_gender_ratio(year_record_list)
        svgInsert = svgCodeGenerator(year_record_list,year_start_input,year_end_input)
        svgFile = svgFileGenerator(svgInsert,str(year_start_input)+str(year_end_input),year_record_list)
        context['maps'][str(year_start_input)] = {}
        context['maps'][str(year_start_input)]['map_svg_code'] = svgFile
        if year_record_list.count() > 0:
            context['maps'][str(year_start_input)]['sex'] = year_record_list[0].sex
...
def svgCodeGenerator(record_list, year_start_input, year_end_input):

    if year_start_input == year_end_input:
        uniquifier = year_start_input
    ...
    # for record in record_list:
        ...
        # OLD WAY -- made obsolete (?) Mar 3 2023
    ...
    number_of_years = year_end_input - year_start_input + 1
    for state in all50States:
        # print("Working on...",state)
        svgInsert = svgInsert + "@keyframes anim_" + state.lower() + str(year_start_input) + " {"
        for year in range(year_start_input,year_end_input+1):
            if record_list.filter(year=year, state=state).count() > 0:
                rspd = record_list.filter(year=year, state=state)[0].rspd
                if rspd >= 1.75:
                    fill_code = "bf0000"
                elif rspd >= 1.25:
                    fill_code = "ff0000"
                ...
                fill_code = "404040"
            svgInsert = svgInsert + str((year-year_start_input)*100/number_of_years) + "% {fill:#" + fill_code + "} "
        svgInsert = svgInsert + "}\n." + state.lower() + str(year_start_input) + " {animation-name: anim_" + state.lower() + str(year_start_input) + "; animation-duration: " + str(number_of_years) + "s; animation-iteration-count: infinite; animation-timing-function: step-end;} \n"
    # svgInsert = svgInsert + "@keyframes anim_name_year_title {"
    # for year in range(year_start_input,year_end_input+1):
        # svgInsert = svgInsert + str((year-year_start_input)*100/number_of_years) + "% {content: \"" + str(year) + "\"} "
    # svgInsert = svgInsert + "} \n.experimental::after {animation-name: anim_name_year_title; animation-duration: " + str(number_of_years) + "s; animation-iteration-count: infinite; animation-timing-function: step-end;} \n"
    # svgInsert = svgInsert + " @keyframes anim_experimental {\n0.0% {\ncontent: '1944';}\n50.0% {\ncontent: '1945';\n}\n} \n .experimental::after {\n animation-name: anim_experimental; animation-duration: 2s; animation-iteration-count: infinite; animation-timing-function: step-end;} "

    # svgInsert = svgInsert + "@keyframes anim_ak" + uniquifier + " {0% {fill:#ff0000} 33% {fill:#00ff00} 67% {fill:#0000ff}}\n"
    # svgInsert = svgInsert + ".ak" + uniquifier + " {animation-name: anim_ak" + uniquifier + ";animation-duration: 3s;animation-iteration-count: infinite;}\n"
    # <animate attributeName="fill" dur="2s" repeatDur="indefinite" calcMode="discrete" values="#ff0000;#00ff00;#0000ff" keyTimes="0;0.333;0.667" />
    # svgInsert = svgInsert + "\n\n"
...
def svgFileGenerator(svgInsert, uniquifier, record_list):
    ...
    codePartTwo = codePartTwo + '</svg></svg>'
