...
def index(request):
    ...
    if request.method == 'GET':
        ...
        if "submitButtonForm1" in request.POST:
            context = map_from_one_name_NYYS(request, sfname, int(syear1), int(syear2), ssex)
...
def map_from_one_name_NYYS(request, name_input, year_start_input, year_end_input, sex_input):
...
def svgCodeGenerator(record_list, uniquifier):
    ...
    for state in all50States:
        svgInsert = svgInsert + "@keyframes anim_" + state.lower() + uniquifier + " {0% {fill:#ff0000} 33% {fill:#00ff00} 67% {fill:#0000ff}}\n"
        svgInsert = svgInsert + "." + state.lower() + uniquifier + " {animation-name: anim_" + state.lower() + uniquifier + ";animation-duration: 3s;animation-iteration-count: infinite;animation-timing-function: step-end;}\n"
